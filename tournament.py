...
def load_agents_from_directory(
    ...
    Return a list of (iteration, agent, directory).
    """
    agents = []
...
def test_mode(directories: List[str], game_class: str, agent_class: str, limit: int = None):
    ...
    """
    agents = load_agents_from_directory(directories, game_class, agent_class, limit=limit)
    if len(agents) < 1:
        print("No agents found.")
        return

    print(f"Found {len(agents)} agents across directories:")
    for iteration, _, directory in agents:
